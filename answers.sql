...
SELECT models.name, brands.name, headquarters FROM models JOIN brands ON models.brand_id = brands.brand_id WHERE 
brands.name = 'Ford' AND models.name = 'Mustang';
...
SELECT brands.name, models.year, models.name FROM brands JOIN models ON brands.brand_id = models.brand_id
...
SELECT b.name, m.name, m.year, b.discontinued, b.discontinued-m.year AS "years_unil_brand_discontinued"
FROM models AS m
...
SELECT models.brand_id FROM models GROUP BY models.brand_id HAVING COUNT(models.brand_id) > 5;
